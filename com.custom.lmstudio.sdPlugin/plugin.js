...
function generateStatusSvg(
  ...
  // Color scheme matching redesigned icons
  ...
  // Split name for 2 lines if needed (slightly more chars per line)
...
async function loadModel(modelPath, config = {}) {
  try {
    ...
    if (config && Object.keys(config).length > 0 && !config.useLastSettings) {
      ...
      debugLog(
        `Load with config body: ${JSON.stringify(response.data).substring(0, 1000)}${JSON.stringify(response.data).length > 1000 ? "..." : ""}`,
      );
      ...
      debugLog(
        `Alt payload body: ${JSON.stringify(altResp.data).substring(0, 1000)}${JSON.stringify(altResp.data).length > 1000 ? "..." : ""}`,
      );
    ...
    debugLog(
      `Load without config body: ${JSON.stringify(response2.data).substring(0, 1000)}${JSON.stringify(response2.data).length > 1000 ? "..." : ""}`,
    );
    ...
    const errDetail =
      response2.data && response2.data.error
        ? response2.data.error
        : `API returned status ${response2.status}`;
...
async function handleLoadModel(context, settings) {
  ...
  const useLastSettings =
    settings.useLastSettings === true || settings.useLastSettings === "true";
...
async function handleQuickChat(context, settings) {
  ...
  const copyToClipboard =
    settings.copyToClipboard === true || settings.copyToClipboard === "true";
...
async function handleProcessClipboard(context, settings) {
  ...
  if (
    ...
    setTimeout(() => {
      if (contexts[context]) {
        const title =
          contexts[context].settings.model_name_display || "Clipboard Action";
...
function onWillAppear(context, settings, action, device) {
  ...
  if (action === ACTIONS.TOGGLE_SERVER) {
    ...
    action === ACTIONS.LOAD_MODEL ||
    action === ACTIONS.PROCESS_CLIPBOARD
  ) {
